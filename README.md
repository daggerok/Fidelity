...
Fidelity ETF holdings to Watchlist. A single-file client-side tool that reads the generated `./api/fidelity` static feed (SEC EDGAR N-PORT-P holdings, Yahoo Finance price history and distributions) into a searchable ETF/category catalog with per-fund tabs, watchlist aggregation, N-PORT upload, ticker copy and CSV/TXT export — the same look and feel as the sibling applications.
...
| --- | --: | --- |
| `MAX_FETCHES` | all | Batch size: with a positive value the updater continues after the committed cursor in `api/fidelity/update-state.json`; empty or `0` is a full pass — every fund is refreshed in one run. |
...
| `TICKERS` | all | Space-, comma- or semicolon-separated ticker allowlist, e.g. `FTEC FDVV FDIS FCOM`. |
...
TICKERS="FTEC FDVV FDIS FCOM" ./scripts/update-data.ts
